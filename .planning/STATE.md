...
## Активная сессия: 2026-07-22

### Что сделано (2026-07-21/22)

1. ✅ **Perf раунд 3-4: PSI 75 → ~85 (локально 88)**
   - Hero: h1/фото видимы сразу в prerender (убран entrance-анимация opacity-0) — LCP delay 2143→650ms
   - Скелетоны lazy-секций: точные min-height по замеренным высотам → CLS 0.44→0
   - Шрифты: `fonts.css` (16K) сначала eager, потом заинлайнен в index.html; preload Onest cyrillic woff2 — font-swap CLS убит
   - Hero-фото: responsive srcset 700w/1400w (103KB→27KB, preload с imagesrcset), slider q70 (80→52KB)
   - Реальный CLS в браузере = 0 (проверено PerformanceObserver); PSI иногда показывает флаки

2. ✅ **Проверка хостинга на лишние файлы**
   - Найден старый `/dist.zip` (3.2MB) в корне — удалён пользователем
   - `.git`, `.env`, `*.bak`, `composer.json` — 403; остальное — SPA fallback (не файлы)
   - Открыты по-прежнему: `content/*.backup.json` (low, черновики CMS)

3. ✅ **Mobile UX правки**
   - Заголовки с переносами: «ГИЛЬДИЯ/ФОТОГРАФОВ/САН ЛАЙФ - ЭТО», «ФОТОСЛУЖБА/САН ЛАЙФ -/ПРЕДЛОЖЕНИЕ О/СОТРУДНИЧЕСТВЕ»
   - Отзывы: свайп пальцем в партнёрском слайдере + автопрокрутка 5с на обоих (пауза при hover, off при reduced-motion)
   - BackToTop: одна кнопка над нижней панелью; скрывается при открытом меню мессенджеров (custom event `sunlife:contact-menu`)
   - Меню мессенджеров: исправлен тоггл — ✕ теперь закрывает (menuRef был узкий: click-outside переоткрывал)
   - Отступы: шапка py-3→py-2, нижняя панель pb 1rem→0.5rem

## Предыдущая сессия: 2026-07-21
...
### Следующий шаг
1. Задеплоить свежий `dist.zip` через `deploy.php?key=<DEPLOY_KEY из api/config.local.php>` (в нём: perf-фиксы 3-4 раунда, mobile UX, фикс тоггла меню).
2. Вписать `SMTP_PASS` в `api/config.local.php` на сервере → проверить приход копии на почту.
3. Включить «Обрабатывать статику через Apache» в панели hostiman (кэш-заголовки + закроет .json/бэкапы), иначе — тикет в поддержку.
4. Дальше по бэклогу: todo-011 (privacy), todo-005 (Метрика — нужен ID), сабсеттинг шрифтов под ₽ (LCP -50KB в критпути).

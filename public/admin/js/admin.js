...
function renderEditor(file, data) {
  ...
  switch (file) {
    case 'pricing':
      renderPricingEditor(container, data);
      break;
    case 'faq':
      renderFAQEditor(container, data);
      break;
    case 'gallery':
      renderGalleryEditor(container, data);
      break;
    case 'benefits':
      renderBenefitsEditor(container, data);
      break;
    case 'reviews':
      renderReviewsEditor(container, data);
      break;
    case 'steps':
      renderStepsEditor(container, data);
      break;
    case 'geography':
      renderGeographyEditor(container, data);
      break;
    case 'partnership':
      renderPartnershipEditor(container, data);
      break;
    case 'meta':
      renderMetaEditor(container, data);
      break;
    default:
...
function renderPricingEditor(container, data) {
  ...
  let html = `
    <div class="card">
      ...
      <div class="table-container">
        <table>
          <thead>
            <tr><th>Название</th><th>Цена</th><th>Описание</th><th>Популярный</th><th>Действия</th></tr>
          </thead>
          <tbody>
  `;
      ...
      <tr>
        <td colspan="5">
          <div class="form-group">
            <label>Особенности (одна на строку):</label>
            <textarea class="pkg-features" data-idx="${i}" rows="3">${pkg.features.map(f => f.text).join('\n')}</textarea>
          </div>
  ...
  html += `
      ...
      <div class="mt-2 text-right">
        <button class="btn btn-primary" onclick="savePricing()">Сохранить изменения</button>
      </div>
    </div>
  `;
...
function savePricing() {
  const rows = document.querySelectorAll('.pkg-name');
  rows.forEach(input => {
...
function renderFAQEditor(container, data) {
  ...
  let html = `
    ...
    <div id="faqCustomer" class="tab-panel active">
      ${renderFAQList(categories.customer || [], 'customer')}
    </div>
    <div id="faqPartnership" class="tab-panel">
      ${renderFAQList(categories.partnership || [], 'partnership')}
    </div>
    <div class="mt-2 text-right">
      <button class="btn btn-primary" onclick="saveFAQ()">Сохранить изменения</button>
    </div>
  `;
  container.innerHTML = html;
}
    ...
    html += `
      <div class="accordion-item open" data-cat="${category}" data-idx="${i}">
        ...
        <div class="accordion-body">
          <div class="form-group">
            <label>Вопрос</label>
            <input type="text" class="faq-q" data-cat="${category}" data-idx="${i}" value="${escapeHtml(item.question)}">
          </div>
          <div class="form-group">
            <label>Ответ</label>
            <textarea class="faq-a" data-cat="${category}" data-idx="${i}" rows="4">${escapeHtml(item.answer)}</textarea>
          </div>
        </div>
      </div>
    `;
...
function addFAQ(category) {
  ...
  currentData.categories[category].push({
    id: 'faq-' + Date.now(),
...
function saveFAQ() {
  ['customer', 'partnership'].forEach(cat => {
    ...
    document.querySelectorAll(`.faq-q[data-cat="${cat}"]`).forEach(q => {
      ...
      items.push({
        id: currentData.categories[cat][idx]?.id || 'faq-' + Date.now() + '-' + idx,
        question: q.value,
        answer: document.querySelector(`.faq-a[data-cat="${cat}"][data-idx="${idx}"]`).value,
      });
    });
...
function renderGalleryEditor(container, data) {
    ...
    html += `
      <div class="image-card">
        ...
        <div class="image-info">
          <input type="text" class="gallery-alt" data-idx="${i}" value="${escapeHtml(img.alt)}" placeholder="Описание (SEO)" style="width:100%;font-size:0.75rem;padding:0.25rem">
        </div>
        <div class="image-actions">
          <button class="btn btn-danger btn-sm" onclick="deleteGalleryImage(${i})">✕</button>
        </div>
      </div>
    `;
  });
  html += `
      ...
      <div class="mt-2 text-right">
        <button class="btn btn-primary" onclick="saveGallery()">Сохранить изменения</button>
      </div>
    </div>
  `;
  container.innerHTML = html;
...
function renderBenefitsEditor(container, data) {
  ...
  let html = `
    <div class="card">
      <div class="card-header">
        <h3>Преимущества</h3>
        <button class="btn btn-primary btn-sm" onclick="addBenefit()">+ Добавить</button>
      </div>
  `;
    ...
    html += `
      <div class="form-row" style="margin-bottom:1rem;padding-bottom:1rem;border-bottom:1px solid var(--border)">
        <div class="form-group">
          ...
          <input type="text" class="benefit-title" data-idx="${i}" value="${escapeHtml(b.title)}">
        </div>
        <div class="form-group">
          <label>Иконка</label>
          <select class="benefit-icon" data-idx="${i}">
            <option value="camera" ${b.icon === 'camera' ? 'selected' : ''}>Камера</option>
            <option value="tag" ${b.icon === 'tag' ? 'selected' : ''}>Ценник</option>
            <option value="truck" ${b.icon === 'truck' ? 'selected' : ''}>Доставка</option>
            <option value="clock" ${b.icon === 'clock' ? 'selected' : ''}>Часы</option>
          </select>
        </div>
        <div class="form-group" style="flex:2">
          <label>Описание</label>
          <textarea class="benefit-desc" data-idx="${i}" rows="2">${escapeHtml(b.description)}</textarea>
        </div>
        <div style="display:flex;align-items:flex-end">
          <button class="btn btn-danger btn-sm" onclick="deleteBenefit(${i})">Удалить</button>
        </div>
      </div>
    `;
  });
  html += `
      <div class="text-right">
        <button class="btn btn-primary" onclick="saveBenefits()">Сохранить изменения</button>
      </div>
    </div>
  `;
  container.innerHTML = html;
...
function addBenefit() {
  currentData.benefits.push({ id: 'benefit-' + Date.now(), title: '', description: '', icon: 'camera' });
  renderBenefitsEditor(document.getElementById('editorContainer'), currentData);
}

function deleteBenefit(idx) {
  if (!confirm('Удалить?')) return;
  currentData.benefits.splice(idx, 1);
  renderBenefitsEditor(document.getElementById('editorContainer'), currentData);
}
...
function saveBenefits() {
  ...
  document.querySelectorAll('.benefit-title').forEach(input => {
    ...
    benefits.push({
      id: currentData.benefits[idx]?.id || 'benefit-' + Date.now() + '-' + idx,
      title: input.value,
      description: document.querySelector(`.benefit-desc[data-idx="${idx}"]`).value,
      icon: document.querySelector(`.benefit-icon[data-idx="${idx}"]`).value,
    });
  });
...
function renderReviewsEditor(container, data) {
  ...
  let html = `
    <div class="card">
      <div class="card-header">
        <h3>Отзывы</h3>
        <button class="btn btn-primary btn-sm" onclick="addReview()">+ Добавить отзыв</button>
      </div>
  `;
  reviews.forEach((r, i) => {
    html += `
      <div class="form-row" style="margin-bottom:1rem;padding-bottom:1rem;border-bottom:1px solid var(--border)">
        <div class="form-group">
          <label>Автор</label>
          <input type="text" class="review-author" data-idx="${i}" value="${escapeHtml(r.author)}">
        </div>
        <div class="form-group">
          <label>Город (опц.)</label>
          <input type="text" class="review-city" data-idx="${i}" value="${escapeHtml(r.city || '')}">
        </div>
        <div class="form-group">
          <label>Дата (опц.)</label>
          <input type="text" class="review-date" data-idx="${i}" value="${escapeHtml(r.date || '')}">
        </div>
        <div class="form-group" style="flex:2">
          <label>Текст</label>
          <textarea class="review-text" data-idx="${i}" rows="3">${escapeHtml(r.text)}</textarea>
        </div>
        <div style="display:flex;align-items:flex-end">
          <button class="btn btn-danger btn-sm" onclick="deleteReview(${i})">Удалить</button>
        </div>
      </div>
    `;
  });
  html += `
      <div class="text-right">
        <button class="btn btn-primary" onclick="saveReviews()">Сохранить изменения</button>
      </div>
    </div>
  `;
  container.innerHTML = html;
...
function addReview() {
  currentData.reviews.push({ id: 'review-' + Date.now(), author: '', text: '' });
  renderReviewsEditor(document.getElementById('editorContainer'), currentData);
}

function deleteReview(idx) {
  if (!confirm('Удалить отзыв?')) return;
  currentData.reviews.splice(idx, 1);
  renderReviewsEditor(document.getElementById('editorContainer'), currentData);
}
...
function saveReviews() {
  ...
  document.querySelectorAll('.review-author').forEach(input => {
    const idx = input.dataset.idx;
    reviews.push({
      id: currentData.reviews[idx]?.id || 'review-' + Date.now() + '-' + idx,
      author: input.value,
      text: document.querySelector(`.review-text[data-idx="${idx}"]`).value,
      city: document.querySelector(`.review-city[data-idx="${idx}"]`).value || undefined,
      date: document.querySelector(`.review-date[data-idx="${idx}"]`).value || undefined,
    });
  });
...
function renderStepsEditor(container, data) {
  let html = `
    <div class="card">
      <div class="card-header"><h3>Почему выбирают нас</h3></div>
      <div class="form-group">
        <label>Вводный текст</label>
        <textarea id="stepsIntro" rows="2">${escapeHtml(data.introText || '')}</textarea>
      </div>
  `;
  (data.steps || []).forEach((s, i) => {
    html += `
      <div class="form-row" style="margin-bottom:1rem;padding-bottom:1rem;border-bottom:1px solid var(--border)">
        <div class="form-group">
          <label>Заголовок</label>
          <input type="text" class="step-title" data-idx="${i}" value="${escapeHtml(s.title)}">
        </div>
        <div class="form-group">
          <label>Статистика</label>
          <input type="text" class="step-stat" data-idx="${i}" value="${escapeHtml(s.stat || '')}">
        </div>
        <div class="form-group" style="flex:2">
          <label>Описание</label>
          <textarea class="step-desc" data-idx="${i}" rows="2">${escapeHtml(s.description)}</textarea>
        </div>
      </div>
    `;
  });
  html += `
      <div class="text-right">
        <button class="btn btn-primary" onclick="saveSteps()">Сохранить изменения</button>
      </div>
    </div>
  `;
  container.innerHTML = html;
...
function saveSteps() {
  ...
  document.querySelectorAll('.step-title').forEach(input => {
    const idx = input.dataset.idx;
    steps.push({
      id: currentData.steps[idx]?.id || 'step-' + Date.now() + '-' + idx,
      title: input.value,
      description: document.querySelector(`.step-desc[data-idx="${idx}"]`).value,
      image: currentData.steps[idx]?.image || '/images/about-1.avif',
      stat: document.querySelector(`.step-stat[data-idx="${idx}"]`).value || undefined,
    });
  });
...
function renderGeographyEditor(container, data) {
  let html = `
    <div class="card">
      <div class="card-header"><h3>География</h3></div>
      <div class="form-group">
        <label>Текст о географии</label>
        <textarea id="geoText" rows="3">${escapeHtml(data.geographyText || '')}</textarea>
      </div>
      <h4 class="mb-1">Города</h4>
  `;
  (data.cities || []).forEach((c, i) => {
    html += `
      <div class="form-row" style="margin-bottom:0.5rem">
        <div class="form-group">
          <input type="text" class="city-name" data-idx="${i}" value="${escapeHtml(c.name)}" placeholder="Город">
        </div>
        <div class="form-group">
          <input type="text" class="city-region" data-idx="${i}" value="${escapeHtml(c.region || '')}" placeholder="Регион (опц.)">
        </div>
        <div>
          <button class="btn btn-danger btn-sm" onclick="deleteCity(${i})">Удалить</button>
        </div>
      </div>
    `;
  });
  html += `
      <button class="btn btn-secondary btn-sm mb-2" onclick="addCity()">+ Добавить город</button>
      <div class="text-right">
        <button class="btn btn-primary" onclick="saveGeography()">Сохранить изменения</button>
      </div>
    </div>
  `;
  container.innerHTML = html;
...
function addCity() {
  currentData.cities.push({ name: 'Новый город' });
  renderGeographyEditor(document.getElementById('editorContainer'), currentData);
}

function deleteCity(idx) {
  currentData.cities.splice(idx, 1);
  renderGeographyEditor(document.getElementById('editorContainer'), currentData);
}
...
function renderPartnershipEditor(container, data) {
  let html = `
    ...
    <div class="mt-2 text-right">
      <button class="btn btn-primary" onclick="savePartnership()">Сохранить изменения</button>
    </div>
  `;
  container.innerHTML = html;
}
  ...
  items.forEach((item, i) => {
    html += `
      <div class="form-row" style="margin-bottom:0.75rem">
        <div class="form-group"><input type="text" class="about-num" data-idx="${i}" value="${escapeHtml(item.number)}" placeholder="№"></div>
        <div class="form-group" style="flex:3"><input type="text" class="about-text" data-idx="${i}" value="${escapeHtml(item.text)}" placeholder="Текст"></div>
        <div><button class="btn btn-danger btn-sm" onclick="deleteAboutItem(${i})">Удалить</button></div>
      </div>
    `;
  });
  return html;
...
function renderPartnershipOffers(items) {
  ...
  items.forEach((item, i) => {
    html += `
      <div style="margin-bottom:1rem;padding:1rem;border:1px solid var(--border);border-radius:var(--radius-sm)">
        <div class="form-row">
          <div class="form-group"><input type="text" class="offer-num" data-idx="${i}" value="${escapeHtml(item.number)}" placeholder="№"></div>
          <div class="form-group" style="flex:2"><input type="text" class="offer-title" data-idx="${i}" value="${escapeHtml(item.title)}" placeholder="Заголовок"></div>
          <div><button class="btn btn-danger btn-sm" onclick="deleteOfferItem(${i})">Удалить</button></div>
        </div>
        <div class="form-group">
          <textarea class="offer-desc" data-idx="${i}" rows="2" placeholder="Описание">${escapeHtml(item.description)}</textarea>
        </div>
      </div>
    `;
  });
...
function renderPartnershipPrices(items) {
  ...
  items.forEach((item, i) => {
    html += `
      <div class="form-row" style="margin-bottom:0.75rem">
        <div class="form-group" style="flex:2"><input type="text" class="price-title" data-idx="${i}" value="${escapeHtml(item.title)}" placeholder="Название"></div>
        <div class="form-group"><input type="text" class="price-value" data-idx="${i}" value="${escapeHtml(item.price)}" placeholder="Цена"></div>
        <div><button class="btn btn-danger btn-sm" onclick="deletePriceItem(${i})">Удалить</button></div>
      </div>
    `;
  });
  return html;
...
function renderPartnershipTeam(items) {
  ...
  items.forEach((item, i) => {
    html += `
      <div class="form-row" style="margin-bottom:0.75rem">
        <div class="form-group"><input type="text" class="team-name" data-idx="${i}" value="${escapeHtml(item.name)}" placeholder="Имя"></div>
        <div class="form-group"><input type="text" class="team-role" data-idx="${i}" value="${escapeHtml(item.role)}" placeholder="Роль"></div>
        <div><button class="btn btn-danger btn-sm" onclick="deleteTeamItem(${i})">Удалить</button></div>
      </div>
    `;
  });
  return html;
...
function renderMetaEditor(container, data) {
  let html = `
    <div class="card">
      <div class="card-header"><h3>SEO — Основные настройки</h3></div>
      <div class="form-group">
        <label>Название сайта</label>
        <input type="text" id="siteName" value="${escapeHtml(data.siteMeta?.siteName || '')}">
      </div>
      <div class="form-group">
        <label>URL сайта</label>
        <input type="text" id="baseUrl" value="${escapeHtml(data.siteMeta?.baseUrl || '')}">
      </div>
    </div>
  `;
  (data.pages || []).forEach((page, i) => {
    html += `
      <div class="card" style="margin-top:1rem">
        <div class="card-header"><h3>Страница: ${escapeHtml(page.path)}</h3></div>
        <div class="form-row">
          <div class="form-group"><label>Title</label><input type="text" class="page-title" data-idx="${i}" value="${escapeHtml(page.title)}"></div>
          <div class="form-group"><label>OG Title</label><input type="text" class="page-ogtitle" data-idx="${i}" value="${escapeHtml(page.ogTitle)}"></div>
        </div>
        <div class="form-group"><label>Description</label><textarea class="page-desc" data-idx="${i}" rows="2">${escapeHtml(page.description)}</textarea></div>
        <div class="form-group"><label>OG Description</label><textarea class="page-ogdesc" data-idx="${i}" rows="2">${escapeHtml(page.ogDescription)}</textarea></div>
        <div class="form-row">
          <div class="form-group"><label>OG Image</label><input type="text" class="page-ogimg" data-idx="${i}" value="${escapeHtml(page.ogImage)}"></div>
          <div class="form-group"><label>Canonical</label><input type="text" class="page-canonical" data-idx="${i}" value="${escapeHtml(page.canonical)}"></div>
        </div>
      </div>
    `;
  });
  html += `
    <div class="mt-2 text-right">
      <button class="btn btn-primary" onclick="saveMeta()">Сохранить изменения</button>
    </div>
  `;
  container.innerHTML = html;
}
...
function saveMeta() {
  currentData.siteMeta = {
    siteName: document.getElementById('siteName').value,
    baseUrl: document.getElementById('baseUrl').value,
    defaultOgImage: currentData.siteMeta?.defaultOgImage || '/images/og-default.jpg',
  };
  const pages = [];
  document.querySelectorAll('.page-title').forEach((el, i) => {
    pages.push({
      path: currentData.pages[i].path,
      title: el.value,
      description: document.querySelectorAll('.page-desc')[i].value,
      ogTitle: document.querySelectorAll('.page-ogtitle')[i].value,
      ogDescription: document.querySelectorAll('.page-ogdesc')[i].value,
      ogImage: document.querySelectorAll('.page-ogimg')[i].value,
      canonical: document.querySelectorAll('.page-canonical')[i].value,
    });
  });
...
document.addEventListener('DOMContentLoaded', async () => {
  if (document.getElementById('editorContainer')) {
    ...
    // Set up nav links
    document.querySelectorAll('.sidebar-nav a[data-file]').forEach(link => {
